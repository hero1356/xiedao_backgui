    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->comboBox->insertItem(5,icon," 5号打印机");
...
void MainWindow::slot_conbox_index_chaged(int index)
{
    ...
    switch (select_index)
    {
        ...
        case PRINTER_FOUR:
            ...
            break;
    case PRINTER_FIVE:
        for(int i=0; i<msg_list_5->count(); i++)
            current_info_show.append(msg_list_5->at(i));
        break;
        default:
            break;
    }
...
void MainWindow::timer_out()
{
    ...
    if(nwthread->connection_tcp() == false)
    {
        connect_state->setStyleSheet("color: red");
        connect_state->setText("服务器连接失败！");
    }
...
void MainWindow::data_show(QString str )
{
    ...
    {
        if(msg_list_5->count()>MAX_LIST)  msg_list_5->pop_front();
        msg_list_5->append(info);
    }
    else
    {
        if(msg_list_1->count()>MAX_LIST)  msg_list_1->pop_front();
